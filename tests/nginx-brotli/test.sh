...
wait_for_container "$container_id" curl -sf --max-time 1 "http://localhost:8080/brotli"

resp=$(curl -i -H "Accept-Encoding: br" --compressed "http://localhost:8080/brotli")
echo "$resp" | grep -q "Content-Encoding: br" || fail "brotli support test"