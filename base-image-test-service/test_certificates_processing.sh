...
docker run -v "${TEST_DIR}":/tmp/cert/ -e CERTIFICATE_FILE_PASSWORD=testit --rm "${JAVA_BASE_IMAGE}" \
       keytool -v -list -cacerts -storepass testit -alias testcert_000.pem
...
for cn in 'testcerts.com' 'valid1' 'valid2'; do
  <processed.crt awk -v decoder='openssl x509 -noout -subject -enddate 2>/dev/null' '/BEGIN/{close(decoder)};{print | decoder}' | grep $cn;
done      
